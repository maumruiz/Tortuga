class ConstantTable:
    ...
    def add_constant(self, address, value):
        if address >= ConstantTable.INT_CONSTANT_BASE and address < ConstantTable.FLOAT_CONSTANT_BASE:
            self.table['int'][translate_address(address, ConstantTable.INT_CONSTANT_BASE)] = value
        elif address >= ConstantTable.FLOAT_CONSTANT_BASE and address < ConstantTable.STRING_CONSTANT_BASE:
            self.table['float'][translate_address(address, ConstantTable.FLOAT_CONSTANT_BASE)] = value
        elif address >= ConstantTable.STRING_CONSTANT_BASE and address < ConstantTable.INT_BASE:
            self.table['string'][translate_address(address, ConstantTable.STRING_CONSTANT_BASE)] = value
        else:
            print('Error de ejecución: Dirección virtual desconocida')
            return None

    def get_int_value(self, address):
        return self.table['int'][translate_address(address, ConstantTable.INT_CONSTANT_BASE)]
    
    def get_float_value(self, address):
        return self.table['float'][translate_address(address, ConstantTable.FLOAT_CONSTANT_BASE)]

    def get_string_value(self, address):
        return self.table['string'][translate_address(address, ConstantTable.BOOL_CONSTANT_BASE)]
